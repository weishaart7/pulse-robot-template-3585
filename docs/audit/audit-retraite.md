...
- [calculTrimestres.ts](src/lib/retraite/calculTrimestres.ts) — **règle du chômage non indemnisé non implémentée, décision explicite de ne pas la coder cette session** (recherchée et confirmée le 2026-08-11). Contrairement au chômage indemnisé (ratio simple jours/50), le chômage non indemnisé suit un mécanisme forfaitaire et contextuel : 1ʳᵉ période non indemnisée de la carrière → jusqu'à 6 trimestres sans ratio jours/trimestre ; périodes suivantes → prises en compte seulement si elles succèdent sans interruption à une période indemnisée, limite 1 an (4 trimestres), portée à 5 ans si ≥ 20 ans de cotisation et ≥ 55 ans à la cessation du revenu de remplacement ; toute activité, même partielle, interrompt la validation. Cette règle nécessite un état de carrière entière (première période ou non, âge, historique de cotisation) — incompatible avec le traitement période par période du reste de la fonction. `type_activite = 'chomage'` ne distingue de toute façon pas indemnisé de non indemnisé dans le modèle de données actuel (confirmé sur les données réelles de Titouan Weishaar : les libellés "CHÔMAGE" et "CHÔMAGE NON INDEMNISÉ" coexistent, tous deux `type_activite = 'chomage'` — cf. docs/audit/cartographie-trimestres-cotises.md §1.2), donc la fonction applique la règle du chômage indemnisé (50 jours) à toutes les périodes `chomage` sans exception, ce qui **surestime probablement** les trimestres assimilés pour les périodes de chômage non indemnisé (plafond réel bien plus restrictif que le simple ratio jours/50).
...
- `micro_entrepreneur` entièrement exclu (ni cotisé, ni assimilé, ni compté) — sous-estime le total pour tout client ayant une activité micro-entrepreneur significative. Bloqué par deux décisions non tranchées : conversion CA → assiette sociale, barème d'abattement forfaitaire par sous-type BIC/BNC.
- Chômage **non indemnisé** non implémenté (mécanisme forfaitaire/contextuel incompatible avec un traitement période par période, cf. entrée ci-dessus) — `type_activite='chomage'` ne le distingue de toute façon pas de l'indemnisé dans le modèle de données actuel, donc la règle indemnisée (50 jours) est appliquée par défaut à toutes les périodes `chomage`, surestimant probablement l'assimilé non indemnisé.
- Chronologie de franchissement des seuils pour la surcote (âge légal par génération + durée requise) non résolue — dépend d'une granularité temporelle par trimestre qui n'existe pas dans le modèle de données, et l'âge légal lui-même reste instable (LFSS 2026, générations 1964-1968 suspendues jusqu'en 2028).
- Fonction **non branchée** dans `decoteSurTrimestres()`, `minimumContributif()`, ni dans aucun écran — production de la fonction et de ses tests uniquement à ce stade, le branchement fera l'objet d'une décision et d'un commit séparés.
