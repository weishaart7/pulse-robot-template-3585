...
| 16 | Double comptage RAFP possible à l'import RIS (panier `regimesPoints`, non exclu comme CNAVPL/SRE/CNRACL) — actif, non corrigé | [Carriere.tsx:396,415,571-577](src/components/retraite/Carriere.tsx), [regimesSaisieManuelle.ts:25-26](src/lib/retraite/regimesSaisieManuelle.ts) | §7.1, §8 (table des régimes) | Moyenne |
...
**#12 — Décote fonction publique : taux figé sans barème par année d'ouverture des droits.** Le référentiel (§7.3) distingue le taux de décote selon l'année d'ouverture des droits (0,75 % en 2011 jusqu'à 1,25 % à partir de 2015), millésime figé indépendant de la date de liquidation. `decoteSurAgeFonctionPublique()` ([calculFonctionPublique.ts:58-67](src/lib/retraite/calculFonctionPublique.ts)) applique `1.25` en dur, sans paramètre d'année ni commentaire signalant l'hypothèse (à la différence du reste du module, où ce type de simplification est systématiquement documenté en tête de fonction). Impact pratique limité aujourd'hui (les droits ouverts avant 2015 sont marginaux pour un outil utilisé en 2026) mais absence de garde-fou.
...
- **Volet NBI (§7.7) : toujours ouvert.** Recherche exhaustive (`NBI`, `Nouvelle Bonification`, `Bonification Indiciaire`) dans `src/lib/retraite/calculFonctionPublique.ts` et `src/components/retraite/CarriereFonctionPublique.tsx` : aucune occurrence. Aucune fonction de calcul, aucun champ de saisie de points NBI, aucun affichage. L'écart #13 se réduit donc désormais au seul supplément NBI.

**#16 — Double comptage RAFP possible à l'import RIS (actif, non corrigé).** Constat ajouté le 2026-08-15, en suite du signalement en aparté fait en clôturant la correction du double comptage fonction publique/RIS ([correction-double-comptage-fp-ris.md](correction-double-comptage-fp-ris.md), qui notait déjà « comme CNAVPL et RAFP, une saisie manuelle » sans le formaliser comme écart). Diagnostic, pas de correction dans cette session.
...
- **Correction non appliquée dans cette session**, par consigne explicite : ajouter `rafp` au motif `RE_REGIME_SAISIE_MANUELLE` (même mécanisme que la correction SRE/CNRACL du 2026-08-14) suffirait probablement, mais reste à valider et à implémenter dans une session dédiée.
