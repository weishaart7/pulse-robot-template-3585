...
- [calculTrimestres.ts](src/lib/retraite/calculTrimestres.ts) — le plafond de 4 trimestres/an est appliqué séparément au total cotisé et au total assimilé de chaque année (pas de plafond combiné cotisé + assimilé par année civile). Une année où plusieurs catégories se cumulent fortement peut donc afficher plus de 4 trimestres au total sur cette année précise — effet inverse (surestimation locale) de la sous-estimation globale liée à l'exclusion micro-entrepreneur ci-dessus, qui domine largement en pratique.
- [calculTrimestres.ts](src/lib/retraite/calculTrimestres.ts) — `JOURS_PAR_TRIMESTRE_ASSIMILE = 90` (comptage des trimestres assimilés `chomage`/`maladie` par jours écoulés, faute de revenu exploitable pour ces catégories) est une approximation retenue avec l'utilisateur le 2026-08-11, **pas une valeur officielle vérifiée** — à la différence du seuil de validation cotisé (`SEUIL_VALIDATION_TRIMESTRE_PAR_ANNEE`, dérivé des SMIC horaires bruts officiels 2018-2026 et validé explicitement). À affiner si une source CNAV précise sur le décompte des trimestres assimilés est trouvée.
