...
**Pipeline de pension consolidée dupliqué — Synthese.tsx (2026-08-15) — RÉSOLU (2026-08-18).** Historique : [pensionConsolidee.ts](src/lib/retraite/pensionConsolidee.ts) (consommé par [usePensionConsolidee.ts](src/hooks/usePensionConsolidee.ts) pour l'écran Synthèse) avait été extrait en reproduisant fidèlement le pipeline alors codé en ligne dans Carriere.tsx, sans que Carriere.tsx soit refactorée pour le consommer — décision explicite de prudence sur un écran déjà en production, dette documentée ici comme non planifiée.

Fusion faite le 2026-08-18 (cf. [docs/audit/audit-pension-consolidation.md](docs/audit/audit-pension-consolidation.md) pour l'audit préalable et le détail des 3 étapes) : `Carriere.tsx` appelle désormais directement `calculerPensionConsolidee()` sur son propre state live (le pipeline inline a été supprimé), y compris la projection de revenu futur (auparavant réservée à Synthèse). `pensionConsolidee.ts` expose un nouveau champ `detailRegimeGeneral` (décote, surcote, MICO, écrêtement, majoration enfants) pour que Carriere.tsx conserve son affichage détaillé sans recalcul local. La glue de projection (`calculerProjectionRevenuFutur()`) et le chargement date de naissance/liens familiaux (`useProfilFamilialRetraite()`) ont été extraits pour n'exister qu'à un seul endroit, consommés à l'identique par Carriere.tsx et usePensionConsolidee.ts. `CarriereFonctionPublique.tsx`/`CarriereCNAVPL.tsx` gardent leur propre calcul local pour leur affichage de détail intra-carte (formules vérifiées identiques par l'audit, non dupliquées pour le total consolidé). Le champ `autresPensionsMensuelles`, jusque-là saisi sur Carrière mais jamais persisté (donc invisible pour Synthèse), est désormais une colonne `retraite_data.autres_pensions_mensuelles` (migration `20260818000000`), consommée par les deux écrans.
