...
| 13 | ~~Majoration enfants~~ implémentée et branchée depuis (commit `252c331`) ; **supplément NBI fonction publique reste absent — formule désormais sourcée dans le référentiel (§7.7.1), pas encore conçue ni codée** | [CarriereFonctionPublique.tsx:190](src/components/retraite/CarriereFonctionPublique.tsx) (majoration, réglé) · aucune fonction (NBI, formule disponible, conception à faire) | §7.6 (clos), §7.7.1 (formule sourcée, non implémentée) | Moyenne → **partiellement clos** |
...
- **Volet NBI (§7.7) : toujours ouvert côté code, recherche de la formule aboutie le 2026-08-15.** Recherche exhaustive (`NBI`, `Nouvelle Bonification`, `Bonification Indiciaire`) dans `src/lib/retraite/calculFonctionPublique.ts` et `src/components/retraite/CarriereFonctionPublique.tsx` : aucune occurrence. Aucune fonction de calcul, aucun champ de saisie de points NBI, aucun affichage — confirmé à nouveau le 2026-08-15 (recherche élargie à `src/` et `supabase/` entiers, insensible à la casse : toujours aucune occurrence). L'app n'a donc aucune notion de points NBI ni de fonctions ouvrant droit à la NBI.

  Le référentiel (§7.7, avant cette session) ne donnait qu'une description générale de la NBI, sans formule — insuffisant pour coder. Recherche complémentaire menée le 2026-08-15 (diagnostic uniquement, aucun code) : **la formule officielle a été trouvée et sourcée**, citée verbatim depuis Légifrance (Décret n° 2003-1306 du 26 décembre 2003, art. 28, régime CNRACL) — `supplément_NBI = moyenne_annuelle_NBI_revalorisée × trimestres_liquidables_de_perception × (75% / durée_requise_taux_plein)`, sans seuil minimal de perception. Documentée intégralement dans [docs/retraite-base-referentiel.md §7.7.1](../retraite-base-referentiel.md#771-supplément-de-pension-nbi--formule-sourcée), avec les sources secondaires consultées et un point non résolu explicitement signalé : le texte trouvé régit la CNRACL (fonction publique territoriale/hospitalière) ; son application identique au SRE (fonction publique d'État) est corroborée par des sources secondaires (service-public.gouv.fr, CNRACL elle-même) mais pas vérifiée directement dans le code des pensions civiles et militaires de retraite au cours de cette session — à confirmer avant de coder le volet SRE.

  **Aucun code, aucun modèle de données introduit dans cette session**, conformément à la consigne — cette formule sourcée sert de base à une future session de conception (sur le modèle de l'écart #6, surcote parentale), pas encore à une implémentation. L'écart #13 reste donc « partiellement clos » : volet majoration enfants réglé, volet NBI toujours ouvert côté code mais désormais débloqué côté référentiel.
