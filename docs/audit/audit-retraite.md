...
- [calculTrimestres.ts](src/lib/retraite/calculTrimestres.ts) — le plafond de 4 trimestres/an est désormais combiné (cotisé + assimilé confondus par année civile), avec priorité aux trimestres **cotisés** en cas de dépassement combiné (décidée avec l'utilisateur le 2026-08-11). **Cette priorité n'est pas vérifiée par une source officielle** — retenue comme hypothèse de travail favorable au client, sous réserve express de l'utilisateur : « correcte uniquement si le texte/règle de calcul prévoit effectivement cette priorité ». À confirmer si une source CNAV précise sur l'ordre de priorité cotisé/assimilé en cas de cumul est trouvée.
