...
| Sujet | Détail | Source |
|---|---|---|
| **Désynchronisation base ↔ code** | `marital_status.loi_applicable_regime` / `pays_premier_domicile_matrimonial` lus/écrits par le code mais la migration `20260728110000` n'était pas enregistrée en base au moment de l'audit (28/07). **Reconfirmé résolu le 2026-08-07** (vérification directe via `list_tables` sur le projet Supabase, en marge du correctif DUH) : les deux colonnes existent bien en base. | audit-patrimoine §7.1 — **résolu, vérifié 2026-08-07** |
| **Deux nouvelles migrations commitées jamais appliquées en base** (2026-08-07) | Découvert en régénérant les types après la migration `duh_opte` : `marital_status.separation_de_corps` / `separation_corps_clause_renonciation` (migration `20260806120000`, commit `c65fa6b`) et `liberalites.generation_intermediaire_id` (migration `20260806130000`, commit `4e94ab8`) étaient absentes de la base réelle bien que le code (déjà en prod) les lise/écrive (`transmissionHelpers.ts:510`, `DonationForm.tsx`, `liberaliteService.ts`). Toute tentative d'enregistrer une séparation de corps avec clause de renonciation, ou une donation-partage transgénérationnelle, échouait silencieusement ou en erreur. **Corrigé le 2026-08-07** : les deux migrations ont été appliquées (additives, sans risque) et `types.ts` régénéré. Signale un besoin de vérifier plus régulièrement que `supabase/migrations/` et l'état réel du projet restent synchronisés (déjà le même symptôme que la ligne ci-dessus, deux fois en 10 jours). | Repéré lors du correctif DUH (§5.9, 2026-08) — **résolu 2026-08-07** |
...
| **`cashDu` (§6bis) ne soustrait que les donations rapportables — une donation-partage directe à un enfant gonfle `partFinale` sans jamais être exclue de `cashDu`** | Question légale distincte de l'ambiguïté déjà tranchée en §5 ci-dessous (celle-ci portait sur *comment répartir* le résiduel entre plusieurs héritiers simultanément sous-dotés ; celle-ci porte sur *si* un héritier déjà réglé par une donation-partage (non rapportable, art. 1078 : la valeur figée à l'acte n'entre jamais dans `rapportResult.rapports`) doit être compté comme sous-doté du tout. `dejaDetenus` (`index.ts` §6bis, cf. `docs/design-rapport-moins-prenant-2026-08.md`) n'exclut de `cashDu` que les donations *rapportables* (`avance_part`, dans `rapportTotal`) — une donation-partage (`typeImputation === 'partage'`) à un enfant, elle, est bien ajoutée à `partFinale` via `liberalitesMaintenues` mais jamais soustraite de `cashDu` : l'enfant apparaît sous-doté du montant plein de sa donation-partage en plus de sa part théorique, ce qui peut lui faire réclamer une part du résiduel réel qu'il ne devrait pas recevoir (déjà satisfait par la donation-partage en nature). Repéré en creusant le branchement de la donation-partage transgénérationnelle (art. 1078-8, 2026-08) : ex. Marie, 2 enfants, donation-partage de 200 000 € à Aurélien, résiduel réel 300 000 € — `cashDu(Aurélien)` inclut à tort les 200 000 € déjà reçus en nature, en plus de sa part du résiduel. Non corrigé ici, hors périmètre du correctif art. 1078-8 (qui, lui, ne fait qu'aligner le traitement transgénérationnel sur ce même comportement déjà existant pour une donation-partage directe, sans l'aggraver ni le corriger). | Repéré lors du correctif transgénérationnel (2026-08), `docs/design-rapport-moins-prenant-2026-08.md` (mécanisme voisin, question distincte) |
| **`AssuranceVie.tsx` ne passe ni `partageEnvisage` ni `duhOpte` à `computeTransmission`, contrairement aux 3 autres écrans (`ProcessusCalcul.tsx`, `Synthese.tsx`, `Succession2ndDeces.tsx`)** | Repéré en câblant le DUH (§5.9, 2026-08) : ce fichier lit déjà `maritalStatus` (pour `buildFamilyGraph` et `clauses_contrat`) mais ne transmet pas ces deux champs au `ctx`, alors que les 3 autres écrans le font tous les deux. Hypothèse retenue à ce stade (non vérifiée formellement) : l'écran Assurance-vie simule surtout la fiscalité hors succession civile (990 I, clause bénéficiaire), donc l'absence de `partageEnvisage`/`duhOpte` y serait une séparation volontaire plutôt qu'un oubli — mais ce n'est qu'une supposition, à confirmer une fois (voir si l'écran affiche aussi des montants nets successoraux qui, eux, seraient impactés). | Repéré lors du correctif DUH (§5.9, 2026-08) |
| **`Succession2ndDeces.tsx::ctxConjointDecede` ne passe ni `partageEnvisage` ni `duhOpte` ni `conjointOption`, contrairement à `ctxUtilisateurDecede` dans le même fichier** | Repéré en câblant le DUH (§5.9, 2026-08) : le fichier construit deux `TransmissionContext` distincts pour simuler les deux ordres de décès possibles. `ctxUtilisateurDecede` (Utilisateur décède en premier, conjoint survivant) reçoit `partageEnvisage`/`conjointOption`, désormais aussi `duhOpte`. `ctxConjointDecede` (conjoint décède en premier, Utilisateur survivant) ne reçoit aucun des trois — donc si l'Utilisateur (en tant que conjoint survivant dans ce sens de décès) a coché "partage envisagé" ou "DUH optée" dans `Optimisation.tsx`, ce sens de décès simulé ignore silencieusement ces choix. Semble un oubli de symétrie plutôt qu'une séparation volontaire (contrairement au cas AssuranceVie.tsx ci-dessus), mais non corrigé ici — DUH câblé à l'identique de `partageEnvisage` existant, sans en corriger le trou préexistant, conformément à la décision de ne pas élargir le périmètre du correctif. | Repéré lors du correctif DUH (§5.9, 2026-08) |
...
- ~~**Vérifier l'état réel post-fix des migrations** (462777d, 29/07)~~ — confirmé résolu le 2026-08-07 (point §2.1). Deux nouvelles migrations non appliquées découvertes à cette occasion et corrigées le même jour (`separation_de_corps` + `generation_intermediaire_id`, §2.1) — envisager une vérification systématique (`list_migrations` vs `supabase/migrations/`) après chaque session plutôt qu'au hasard d'un correctif suivant.
