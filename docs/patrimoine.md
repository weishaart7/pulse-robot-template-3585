...
**UX1 — Refonte des onglets du formulaire actif (`AssetForm.tsx`).** Objectif : ce formulaire est rempli en autonomie par le client, typiquement en deux temps (saisie complète à la création, puis retouches ponctuelles ensuite) — le besoin identifié portait sur la pertinence et la navigation des champs, pas sur la logique métier (aucun calcul touché). Quatre changements, tous localisés à `AssetForm.tsx` sauf mention contraire : (1) passage de 4 à 5 onglets — `essentiel` / `détention` / `origine` / `caractéristiques` / `charges` — par simple redistribution des champs déjà existants entre onglets, sans ajout ni suppression de champ (détail ligne à ligne dans le diff du composant) ; (2) à la création d'un actif, les onglets autres que "essentiel" sont verrouillés tant que le champ `nature` n'est pas renseigné (rien à qualifier avant ça) et se débloquent dès qu'il l'est, sans verrouillage progressif au-delà de ce seuil ; en édition, tous les onglets restent cliquables dès le montage ; (3) la qualification du bien (`qualification_bien`) est déplacée en tête de l'onglet "origine" et affichée en lecture seule avec sa raison explicative en mode automatique (cas par défaut), la modification manuelle n'étant plus qu'à un clic ("Modifier manuellement") plutôt qu'un `<Select>` systématiquement visible ; (4) le champ "Bien propre par nature" (art. 1404) est désormais masqué pour un utilisateur non en couple — ce correctif réutilise `isInCouple`, exporté depuis `lib/patrimoine/qualification.ts` (auparavant privé au module) plutôt que de dupliquer le critère "marié/pacsé/concubinage" ailleurs. Note technique : l'infrastructure de test composant React (`jsdom`, `@testing-library/react`, `@testing-library/jest-dom`, `@testing-library/user-event`) a été ajoutée à cette occasion, absente jusque-là du projet (seuls des tests de logique pure existaient) ; elle est scopée au seul fichier `AssetForm.test.tsx` via le pragma `// @vitest-environment jsdom`, le reste de la suite continuant de tourner sous l'environnement `node` par défaut (`vitest.config.ts` inchangé). Fichiers touchés : `AssetForm.tsx`, `qualification.ts` (export de `isInCouple`), `AssetForm.test.tsx` (nouveau), `package.json` (devDependencies de test).
