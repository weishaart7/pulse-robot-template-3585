...
**IR1 — La créance de participation aux acquêts est calculée sur des valeurs entièrement découplées des actifs Patrimoine, sans resynchronisation possible.**
`PatrimoineOriginaireSection.tsx` et `PatrimoineFinalSection.tsx` proposent un champ `bienConcerneId` (sélecteur d'actif issu de `useAssets()`) présenté comme « facultatif », mais celui-ci ne sert qu'à afficher un libellé de rappel (`assetLabel`, ligne 46/52) — aucun code n'utilise la sélection d'un actif pour préremplir ou mettre à jour le champ `valeur` (saisie manuelle libre, `<Input type="number">`). Le calcul de la créance (`lib/patrimoine/participationAcquets.ts::computeParticipationAcquets`/`computeAcquetNet`) consomme ensuite ces valeurs `patrimoine_originaire.valeur`/`patrimoine_final.valeur` telles quelles. Si l'utilisateur revalorise l'actif correspondant dans le module Patrimoine après avoir saisi sa ligne de patrimoine originaire/final, rien ne signale l'écart ni ne propose de resynchroniser — la créance de participation calculée peut donc reposer sur une valeur d'actif périmée sans qu'aucun indice visuel n'alerte l'utilisateur. Ce résultat alimente ensuite directement l'assiette fiscale DMTG comme ligne synthétique (`lib/transmission/index.ts:796-806`, `deltaParticipationAcquets`).
Note : le calcul lui-même (`computeAcquetNet`/`computeParticipationAcquets`) est correctement indépendant du mécanisme `qualifierBien`/`getPartSuccessorale`, comme documenté explicitement en tête de `participationAcquets.ts` — pas de double calcul de qualification civile détecté ici, le risque porte uniquement sur la fraîcheur de la valeur saisie.

**IR2 — L'hypothèse « emprunt de société déjà reflété dans la valorisation des parts » n'est vérifiée ni imposée par aucun champ ou aide contextuelle.**
`usePatrimoineCalculations.ts` et `buildPassifLines` (`transmissionHelpers.ts:787-788`) excluent tous deux, de façon strictement identique, les emprunts rattachés à une société (`emprunts.filter(e => !e.societe_id)`) du total des passifs — cf. IC2, cette mécanique d'exclusion est cohérente entre les deux modules. Mais l'hypothèse sous-jacente (la dette de la société est déjà déduite de la `valeur_estimee` — champ `valeurEstimee` — saisie sur la fiche société) n'est appuyée par aucun mécanisme : `SocieteForm.tsx:566-572` est un simple champ numérique libre, sans texte d'aide, sans calcul automatique et sans lien visible vers les emprunts de société existants (`src/pages/societes/SocieteFormPage.tsx`/`SocieteForm.tsx` ne référencent `emprunt` nulle part). Si l'utilisateur saisit la valeur brute de sa société (avant déduction de ses propres emprunts), l'emprunt de société est alors exclu deux fois du patrimoine net affiché : ni comptabilisé dans les passifs du foyer (filtré volontairement), ni déduit de la valeur de la société elle-même.

**IR3 — Sur l'écran Retraite > Épargne, la colonne "Utilisateur" n'est pas la part de l'utilisateur, contrairement à la convention utilisée par Patrimoine.**
`EpargneRetraite.tsx:60-89` réutilise le même moteur que Patrimoine (`getPartSuccessorale`, même convention que `PatrimoineParTeteDetail.tsx::computeByCategory`, cf. commentaire ligne 60-65) pour calculer la part du conjoint (`partConjoint = 1 - getPartSuccessorale(asset)`) sur les actifs PER/assurance-vie du foyer. Mais pour la colonne "utilisateur" (`personne === 'utilisateur'`), `perAssets`/`assuranceVieAssets` restent égaux à `perAssetsFoyer`/`assuranceVieAssetsFoyer` **non filtrés** (ligne 79-84) : le total affiché est celui du foyer entier (100 % des deux, PER de l'utilisateur ET du conjoint), pas la part complémentaire `1 - partConjoint`. Le commentaire du code documente ce choix comme volontaire ("comportement historique, écran sans conjoint identique à avant"), mais cela rompt la symétrie attendue par analogie avec "Patrimoine par tête" (où `userValue + spouseValue = totalValue`) : sur cet écran, colonne "Utilisateur" + colonne "Conjoint" ne font PAS le total du foyer, la colonne "Utilisateur" étant déjà ce total à elle seule. Un utilisateur additionnant les deux colonnes par analogie avec le Résumé Patrimoine surestimerait le patrimoine retraite réel du foyer.

### 🟡 Dette mineure

**ID1 — Le barème 669 CGI est dupliqué dans deux fichiers indépendants, sans source commune.**
`lib/patrimoine/bareme669CGI.ts` (tableau `BAREME_669_CGI`, consommé par le module Patrimoine) et `src/lib/dmtg/params-dmtg.json` (clé `demembrementViager`, consommée par `getDemembrementPct` dans `lib/transmission/index.ts:154-162`, pour la valorisation d'un usufruit/nue-propriété *créé par la succession elle-même* — legs en usufruit, DUH du conjoint, etc., un cas d'usage distinct de IB1). Vérification tranche par tranche : les deux tables sont numériquement identiques à ce jour (9 tranches, mêmes seuils d'âge, mêmes pourcentages 90/10 à 10/90). Mais rien ne dérive l'une de l'autre — une future mise à jour du barème légal (peu fréquente mais déjà arrivée par le passé) devra être répercutée manuellement aux deux endroits, sans garde-fou empêchant une divergence silencieuse entre Patrimoine et Transmission.

**ID2 — Troisième réimplémentation indépendante d'un total "patrimoine net", dans le moteur d'alertes du Dashboard.**
`useAlertesConseil.ts:38-41` recalcule son propre `patrimoineNet` (`assets` bruts − `passifs` − `emprunts` hors société), sans pondération démembrement ni exclusion des éléments à la qualification civile absente — troisième implémentation de ce total aux côtés de `usePatrimoineCalculations.financialSummary` (section 1) et du calcul local de `PatrimoineChart.tsx` (qui, lui, pondère bien par le démembrement). Le commentaire du code reconnaît explicitement l'approximation (« Estimation simple ... suffisante pour un rappel de vigilance plutôt qu'un calcul exact ») et la valeur n'est jamais affichée telle quelle à l'utilisateur — seulement comparée à un seuil (`patrimoineNet > 2 × abattement ligne directe × nombre d'enfants`) pour déclencher une alerte sur la communauté universelle (`lib/alertes/regles.ts:110-113`). Impact limité, mais un ménage détenant un actif démembré ou un bien jamais qualifié pourrait déclencher — ou manquer — cette alerte sur la base d'un total différent de celui affiché dans le Résumé Patrimoine.
