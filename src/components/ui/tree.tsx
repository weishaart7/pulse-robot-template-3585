...
function TreeItemLabel<T = any>({ item: propItem, children, className, ...props }: TreeItemLabelProps<T>) {
  ...
  return (
    <span
      ...
      className={cn(
        'focus-visible:ring-ring/50 bg-background hover:bg-accent data-[selected=true]:bg-accent data-[selected=true]:text-accent-foreground data-[drag-target=true]:bg-accent flex items-center gap-1 rounded-sm px-2 py-1.5 text-sm transition-colors data-[folder=false]:ps-7 focus-visible:ring-[3px] data-[search-match=true]:bg-blue-50 [&_svg]:pointer-events-none [&_svg]:shrink-0',
        className,
      )}
      {...props}
    >
      {item.isFolder() &&
        (toggleIconType === 'plus-minus' ? (
          ...
          <ChevronDownIcon className="text-muted-foreground size-4 aria-[expanded=false]:-rotate-90" />
        ))}
