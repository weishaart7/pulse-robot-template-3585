...
export const FeatureGrid = React.forwardRef<HTMLDivElement, FeatureGridProps>(({
  ...
  return <section ref={ref} className={cn('w-full max-w-6xl mx-auto py-12 md:py-20 px-4', className)}>
        {/* Header Section */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10">
            {categories.map((category, index) => <motion.div key={index} variants={itemVariants} className="flex flex-col items-start">
                ...
                <ul className="space-y-1.5 text-muted-foreground">
                  {category.items.map((item, itemIndex) => <li key={itemIndex}>
                      {item.href ? <a href={item.href} className="hover:text-primary hover:underline underline-offset-2 transition-colors">
                          {item.text}
                        </a> : <span>{item.text}</span>}
                    </li>)}
                </ul>
              </motion.div>)}
          </div>
