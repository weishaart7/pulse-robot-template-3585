      ...
      >
        <CardHeader className="p-0">
          <div className={cn(!displayAvatar && "invisible")}>
            <Avatar
              className="h-12 w-12 ring-2 ring-offset-2 ring-offset-card"
              style={
                {
...
export const RevealCardContainer = forwardRef<HTMLDivElement, RevealCardProps>(
  (
    ...
    // Centre approximatif de l'avatar (bordure 1px + padding carte 24px + rayon avatar 24px)
    const startClip = "circle(38px at 49px 49px)";
    // Unité cohérente avec startClip (px) — mélanger px/% dans le même clip-path
    // empêche GSAP d'interpoler correctement l'animation de révélation.
    const expandClip = "circle(900px at 49px 49px)";
