...
export function SearchBar() {
  ...
  return <div ref={searchRef} className="relative max-w-[400px]">
      <div className="relative">
        <Input placeholder="Rechercher une section..." type="search" value={query} onChange={e => setQuery(e.target.value)} className="peer pe-9 ps-9 mx-0 px-0 h-[34px] focus-visible:ring-[#c2f94f]/30" />
        <div className="pointer-events-none absolute inset-y-0 start-0 flex items-center justify-center ps-3 text-muted-foreground/80 peer-disabled:opacity-50">
          <Search size={16} strokeWidth={2} />
        </div>
        <button className="absolute inset-y-0 end-0 flex h-full w-9 items-center justify-center rounded-e-lg text-muted-foreground/80 outline-offset-2 transition-colors hover:text-foreground focus:z-10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring/70 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50" aria-label="Submit search" type="submit">
