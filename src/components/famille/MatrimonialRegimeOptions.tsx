  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {assets && assets.length > 0 ? <div className="space-y-2">
              {assets.map(asset => <div key={asset.id} className="flex items-center space-x-2">
                  <Checkbox id={asset.id} checked={selectedAssets.includes(asset.id)} onCheckedChange={() => handleAssetToggle(asset.id)} />
...
const PercentageInputs: React.FC<{
  ...
  return <div className="grid grid-cols-2 gap-4 mt-4">
      <div>
        ...
        <Input id="part-pp" type="number" min="0" max="100" value={partPleineProprietee} onChange={e => handlePPChange(e.target.value)} />
      </div>
      <div>
        <Label htmlFor="part-usufruit">Part en usufruit (%)</Label>
        <Input id="part-usufruit" type="number" min="0" max="100" value={partUsufruit} onChange={e => handleUsufruitChange(e.target.value)} />
      </div>
      <div className="col-span-2 text-sm text-muted-foreground">
        Total: {partPleineProprietee + partUsufruit}% {partPleineProprietee + partUsufruit !== 100 && <span className="text-destructive">(doit être égal à 100%)</span>}
      </div>
    </div>;
};
export const MatrimonialRegimeOptions: React.FC<MatrimonialRegimeOptionsProps> = ({
  ...
  useEffect(() => {
    if (maritalData) {
      ...
      if (dataAny.clauses_contrat) {
        try {
          const parsedClauses = typeof dataAny.clauses_contrat === 'string' ? JSON.parse(dataAny.clauses_contrat) : dataAny.clauses_contrat;
      ...
      if (dataAny.donation_dernier_vivant) {
        try {
          const parsedDonation = typeof dataAny.donation_dernier_vivant === 'string' ? JSON.parse(dataAny.donation_dernier_vivant) : dataAny.donation_dernier_vivant;
  ...
  const renderSubClauses = (clauseName: string) => {
    if (clauseName === 'societe_acquets' && clauses[clauseName]?.enabled) {
      return <div className="ml-6 mt-4 space-y-4 border-l-2 border-muted pl-4">
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <Checkbox id="partage_inegal_sub" checked={clauses['partage_inegal_sub']?.enabled || false} onCheckedChange={() => handleClauseToggle('partage_inegal_sub')} />
              <Label htmlFor="partage_inegal_sub">
                Clause de partage inégal (% en PP et % en usufruit)
              </Label>
            </div>
            {clauses['partage_inegal_sub']?.enabled && <PercentageInputs partPleineProprietee={clauses['partage_inegal_sub']?.partPleineProprietee || 50} partUsufruit={clauses['partage_inegal_sub']?.partUsufruit || 50} onChange={(pp, usufruit) => handlePercentageChange('partage_inegal_sub', pp, usufruit)} />}
          </div>

          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <Checkbox id="attribution_integrale_sub" checked={clauses['attribution_integrale_sub']?.enabled || false} onCheckedChange={() => handleClauseToggle('attribution_integrale_sub')} />
              <Label htmlFor="attribution_integrale_sub">
                Clause d'attribution intégrale (% en PP et % en usufruit)
              </Label>
            </div>
            {clauses['attribution_integrale_sub']?.enabled && <PercentageInputs partPleineProprietee={clauses['attribution_integrale_sub']?.partPleineProprietee || 50} partUsufruit={clauses['attribution_integrale_sub']?.partUsufruit || 50} onChange={(pp, usufruit) => handlePercentageChange('attribution_integrale_sub', pp, usufruit)} />}
          </div>

          {regimeType === 'participation_acquets' && <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <Checkbox id="preciput_sub" checked={clauses['preciput_sub']?.enabled || false} onCheckedChange={() => handleClauseToggle('preciput_sub')} />
                <Label htmlFor="preciput_sub">Clause de préciput</Label>
              </div>
              {clauses['preciput_sub']?.enabled && <Button variant="outline" size="sm" onClick={() => handleAssetSelection('preciput_sub')} className="ml-6">
                  Sélectionner les biens
                  {clauses['preciput_sub']?.selectedAssets?.length > 0 && ` (${clauses['preciput_sub'].selectedAssets.length} sélectionné${clauses['preciput_sub'].selectedAssets.length > 1 ? 's' : ''})`}
                </Button>}
            </div>}
        </div>;
    }
  ...
  return <div className="space-y-6">
        ...
        <Dialog open={clauseModalOpen} onOpenChange={setClauseModalOpen}>
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              {clausesForRegime.map(clause => <div key={clause.key} className="space-y-4">
                  <div className="flex items-center space-x-2">
                    <Checkbox id={clause.key} checked={clauses[clause.key]?.enabled || false} onCheckedChange={() => handleClauseToggle(clause.key)} />
                    <Label htmlFor={clause.key} className="flex-1">
                      {clause.label}
                    </Label>
                  </div>

                  {clauses[clause.key]?.enabled && <div className="ml-6 space-y-4">
                      {clause.hasAssets && <Button variant="outline" size="sm" onClick={() => handleAssetSelection(clause.key)}>
                          Sélectionner les biens
                          {clauses[clause.key]?.selectedAssets?.length > 0 && ` (${clauses[clause.key].selectedAssets.length} sélectionné${clauses[clause.key].selectedAssets.length > 1 ? 's' : ''})`}
                        </Button>}

                      {clause.hasOptions && clause.key === 'preciput' && <div className="space-y-2">
                          <div className="flex items-center space-x-2">
                            <Checkbox id={`${clause.key}_pleine_propriete`} checked={clauses[clause.key]?.options?.pleineProprietee || false} onCheckedChange={checked => {
                      const newClauses = {
                        ...clauses,
                        [clause.key]: {
                          ...clauses[clause.key],
                      ...
                      saveClausesData(newClauses, donation);
                          ...
                          <div className="flex items-center space-x-2">
                            <Checkbox id={`${clause.key}_usufruit`} checked={clauses[clause.key]?.options?.usufruit || false} onCheckedChange={checked => {
                      const newClauses = {
                        ...clauses,
                        [clause.key]: {
                          ...clauses[clause.key],
                      ...
                      {clause.hasPercentages && <PercentageInputs partPleineProprietee={clauses[clause.key]?.partPleineProprietee || 50} partUsufruit={clauses[clause.key]?.partUsufruit || 50} onChange={(pp, usufruit) => handlePercentageChange(clause.key, pp, usufruit)} />}
      ...
      <AssetSelectionModal title="Sélectionner les biens" isOpen={assetModalOpen} onClose={() => setAssetModalOpen(false)} onConfirm={handleAssetConfirm} preSelectedAssets={clauses[currentAssetModal]?.selectedAssets || []} />
    </div>;
};